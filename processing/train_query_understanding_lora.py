...
DEFAULT_DATASET_DIR = OUT_DIR / "query_understanding_lora" / "final_5000_dataset"
DEFAULT_OUTPUT_DIR = OUT_DIR / "query_understanding_lora" / "qwen2_5_1_5b_qu_lora_fast"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train retrieval-only Query Understanding LoRA with interruption-friendly defaults. "
            "This is a thin wrapper around processing/train_router_lora.py."
        )
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Resume from the latest checkpoint under --output-dir if one exists. Use --no-auto-resume for a fresh run.",
    )
